%% Terrain Analysis for Small Region of Larsemann Hills
% Author: [Yibo Li]
% Date: [2025-04-16]
...
% Load terrain data
[slope, aspect] = loadTerrainData("slope\slope_small.tif", "Aspect\aspect_small.tif");

%% Wind Rose Visualization
% Generate wind rose plot
generateWindRose(aspect, slope, 'Small Region', 22, ... % MaxFrequency = 22
                'Fig_output\Larsemann_Area6_Rose.png');

%% Aspect Distribution Analysis
% Create aspect distribution visualizations
createAspectVisualizations(aspect, 'Fig_output\aspect_pie_chart6.png');
...
function generateWindRose(aspect, slope, regionName, maxFreq, outputPath)
    ...
    options = {
        ...
        'MaxFrequency', maxFreq, ...     % Maximum frequency percentage
...
function createAspectVisualizations(aspect, outputPath)
    % First create histogram for aspect distribution overview
    figure;
    histogram(aspect, 'BinWidth', 10, 'FaceColor', [0.5 0.5 0.5]);
    title('Aspect Distribution Histogram');
    xlabel('Aspect (degrees)');
    ylabel('Frequency');
    set(gca, 'FontName', 'Arial', 'FontSize', 12);
    
    % Calculate aspect distribution for pie chart
